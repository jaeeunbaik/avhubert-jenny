...
class AVHubertConfig(FairseqDataclass):
    ...
    encoder_layers: int = field(
        default=2, metadata={"help": "num encoder layers in the each block (one sub module of the U-net)"}
    )
...
class AVHubertModel(BaseFairseqModel):
    ...
    def forward_targets(
            self, features: torch.Tensor, mask_indices: Optional[torch.Tensor], target_list: List[torch.Tensor],
